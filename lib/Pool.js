...
class Pool extends EventEmitter {
  ...
  _acquireNext() {
    ...
    /** try to find unused orphan object **/
    for (const [orphanPointer, orphanCallback] of this._orphanResources.entries()) {
      if (orphanCallback === null) {
        this._orphanResources.set(orphanPointer, doCallback);
        return;
      }
    }

    /** There is no idle or orphan resource. We need to create new one **/
   ...
   * @protected
   */
  _createObject(request, _callback) {
    ...
    const handleCallback = (err, obj) => {
      // we are able to provide a parent to orphaned resource
      // via the handler
      if (callback === undefined && this._orphanResources.has(handleCallback)) {
        callback = this._orphanResources.get(handleCallback);
        this._orphanResources.delete(handleCallback);
      }
    ...
    if (callback === undefined) {
      this._orphanResources.set(handleCallback, null);
    }
